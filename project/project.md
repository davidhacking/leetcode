...
- 简介：基于一个玩家完成一场游戏前后，内存应该不存在泄漏的理论，强制进行一次GC，采用两种方式分析是否存在内存泄漏：
  - 通过tracemalloc记录并分析存在的内存泄漏情况，报出的内存泄漏代码节点，在可疑的存在内存泄漏的地方打上log，输出泄漏对象的id值（实际就是程序的虚拟内存地址），通过用C语言编写的内存读取工具直接读取python对象的值，最终确认是否存在内存泄漏
  - 分析遗留在GC中的所有对象间的关系，通过HTML+JS的方式绘制对象间的关系，由于绘制的对象很多都是基本对象，分析难度较大，已弃用
  - 最终解决方案，出于开发效率考虑，通过定时重启game进程的方式，把所有回收内存操作交给了操作系统
- GIL：一个python进程同一时刻只有一个线程运行，CPython实现造成，线程在运行的时候即拿到GIL锁，交出CPU控制权前释放锁，Python的多线程只适合编写非CPU型的应用，例如：IO密集型应用，通过多进程的方式实现并行
- TCMalloc，相比传统的直接使用malloc和free的内存分配策略更加高效，在分配的时候TCMalloc首先检查本地线程缓存中是否有可以分配的内存，如果有则分配这部分内存，如果向操作系统申请了新的内存在应用程序释放时短时间不会释放掉
...
- [Linux并发与同步](http://www.cnblogs.com/vamei/archive/2012/10/09/2715393.html)：**利用互斥锁mutex、条件变量和读写锁构成原子操作**（只有一个线程的原子操作不被乱序，处理竞争条件）**进行同步**，互斥锁就是锁，条件变量的好处在于把check条件成立的操作交给系统完成，系统在检测到条件成立时就唤起等待的线程，读写锁则把锁分成三个状态，共享锁，写锁和释放，这样可以提供系统的并发时的吞吐量，对于条件变量一般py或java实现为wait，notify和notify_all的接口
  - 线程的每个命令是原子执行的，在任何一条命令执行完后都可能产生一次上下文切换
  - 协程，其实就是单线程的生成器，与线程相比省去了一大波上下文切换操作从而更加高效，与线程相比，协程的同步方式是通过消息同步而不是通过共享内存变量
  - 自旋锁(spin_lock)，死等锁，如果锁被占用则等待的进程不断retry，仅适合进入临界区操作的时间极短的情况下，由于省去了线程的上线文切换，所以在线程调度上节省了一部分的资源消耗
- Linux进程间通信：
...
- 物流查询服务：1. 定时更新物流信息；2. 对接所有物流公司的查询接口（菜鸟、圆通）；
  ...
  - **框架：**通过一个星期分表，一个月分库的方法减缓单表单库数据量太大的问题，查询时最热的数据就是当周的数据，通过在订单表中的字段判断时间
- 虚假发货：1. 定义：买家操作了发货，但实际未发货（根据物流信息），根据商品不同有2d和4d发货后的物流信息更新期限
- [MySql物流数据向Hbase转型](http://stor.51cto.com/art/201806/576466.htm)，
  - bloomfilter，对于一个有n个元素的集合，想要判断一个元素x是否在集合中，可以使用k个相互独立的hash函数和m bits的数组判断，bloomfilter有一定的错误率即存在某个不在集合中的元素但是通过bloomfilter方法判断是在的（false positive），如果通过bloomfilter得到一个key不在hbase上，则一定不在hbase上，如果在，则转到相应的regionserver上查找
  - zookeeper，使用Paxos算法解决分布式系统的一致性问题，即zk提供的文件树读取任何一个节点的展示的视图都是一致的，具体如：同步服务，同步配置和命名
    - 每次需要更改zookeeper上的数据时，负责更新的节点都会用自己的提议id向所有人发起提议，其他节点在收到提议后会根据id大小进行拒绝或投票，每个投票只有半数以上的人同意才能通过，通过后集群所有人都将会更新最新数据和提议编号
...
- MySql
  ...
  - master向slave传递binlog来保证master和slave一致，根据业务不同可以划分业务为一致性较高和一致性较低的两种任务，master主要负责写，slave主要分则读，**为什么要分库分表？**当数据量太大时可能某次写数据库需要调整整个B+树，导致访问较慢
- [CAT](https://github.com/dianping/cat)，cat分为客户端和服务端实现，客户端使用ThreadLocal存储打点数据到队列中，上传线程会将消息不断上传到服务器中，客户端数据在打点时如果一直不传数据到服务器会在本地累积，最终挤爆内存，[任何IO都需要做异步，如果不做都可能在高并发时产生瓶颈](https://github.com/dianping/cat/wiki/client)。由于打点数据需要持久化，所以服务器端使用HDFS进行消息存储，客户端的消息传到服务器后会先把消息放到内存队列，然后会开启消费线程读取队列中的消息持久化到HDFS中
...
- Mesos，集群管理工具，master根据slave报告的资源情况分配任务，slave运行docker container执行任务
...
- 简介：TEE操作系统，分离了一部分Android操作系统中敏感的操作权限（例如：身份认证和密钥管理）到TEE中执行，实习IEEE上的OTrP协议完成TA的安装与卸载
  ...
  - 解决了什么问题
    - 内存泄漏：C中忘记释放内存，通过python工具分析
    - 内存碎片化：使用的C的json库使内存碎片化，通过parse json string的方式解决，以及使用Android与TEE通信的Shared memory原地解密安全应用